...
test.describe("Lector Review - Comprehensive E2E Tests", () => {
  ...
  test("should navigate between pages", async ({ page }) => {
    ...
    // Accessibility check: Verify navigation buttons have proper attributes
  ...
  test("should enter and persist data in template form", async ({ page }) => {
    ...
    await page
      .waitForSelector(
        'input[placeholder*="Study ID"], input[placeholder*="study"]',
        { timeout: 5000 }
      )
      .catch(() => {});
  ...
  test("should toggle between Template Form and Schema Form", async ({
    page,
  }) => {
    ...
    if (await templateFormButton.isVisible()) {
      ...
      if (await schemaError.isVisible().catch(() => false)) {
        ...
        console.log(
          "Schema form not available - schema.json may not be accessible"
        );
      } else {
  ...
  test("should open Template Manager modal", async ({ page }) => {
    ...
    if (await templateFormButton.isVisible()) {
      ...
      const isActive = await templateFormButton.evaluate((el) =>
    ...
    const manageTemplatesButton = page.getByRole("button", {
      name: /Manage Templates|Manage field templates/i,
    });
    ...
    if (await manageTemplatesButton.isVisible()) {
      ...
      await expect(page.getByText(/Template Manager/i)).toBeVisible({
        timeout: 3000,
      });
  ...
  test("should create a new project", async ({ page }) => {
    ...
    await expect(projectSelect).toContainText("e2e-test-project", {
      timeout: 5000,
    });
  });
  ...
  test("should export JSON", async ({ page }) => {
    ...
    await expect(page.getByText(/Data exported|exported as JSON/i))
  ...
  test("should export CSV", async ({ page }) => {
    ...
    // Verify toast notification appears
    await expect(page.getByText(/Data exported|exported as CSV/i))
      .toBeVisible({ timeout: 3000 })
      .catch(() => {});
  });
  ...
  test("should show toast notifications", async ({ page }) => {
    ...
    await expect(
      page.locator('[class*="toast"], [style*="position: fixed"]').first()
    )
      .toBeVisible({ timeout: 3000 })
      .catch(() => {});

    // Verify toast message is displayed
    const toastMessage = page.getByText(/Data exported|exported as JSON/i);
  ...
  test("should handle errors gracefully", async ({ page }) => {
    ...
    // Attempt to create project with duplicate name (if exists)
    const addButton = page
      .getByRole("button", { name: /Add project/i })
      .first();
    if (await addButton.isVisible()) {
      ...
      // Try empty name or duplicate
      await page.getByPlaceholder("Enter value...").fill("default");
      await page.getByRole("button", { name: "Confirm" }).click();
      await page.waitForTimeout(1000);
  ...
  test("should display highlights list", async ({ page }) => {
    ...
    if (await highlightsSection.isVisible()) {
      ...
      const highlightsList = page
        .locator("ul")
        .filter({ has: highlightsSection });
      const noHighlights = page.getByText(/No highlights yet/i);

      const listVisible = await highlightsList
        .first()
        .isVisible()
        .catch(() => false);
      const noHighlightsVisible = await noHighlights
        .isVisible()
        .catch(() => false);
